...
- `references/project-config.md` — 프로젝트별 단어 사전·규칙(`.yoonmoon.md`) 규약. **Phase 0**에서 설정 파일을 찾으면 읽는다.
...
4. **프로젝트 설정 로드** — 현재 디렉터리에서 상위로 `.yoonmoon.md`를 탐색해 발견하면 읽는다(규약은
   `references/project-config.md`). 보존 사전·치환 사전·금칙어·기본 옵션·예외 규칙을 이번 작업의 우선
   기준으로 삼는다(우선순위: 대화 지시 > `.yoonmoon.md` > 스킬 기본값). 없으면 기본값으로 진행.
5. 작업 시작 시 한 줄 상태를 출력한다: `humanize — 장르: {장르} / 강도: {강도}`{설정 적용 시 ` / 설정: .yoonmoon.md`}
</phase>
...
- **프로젝트 설정 반영**(있으면): 보존 사전 항목은 원형 유지, 치환 사전은 강제 적용, 금칙어는 우선
  제거, 예외 규칙으로 카테고리 심각도를 조정한다. 보존 사전이 치환·금칙어보다 우선한다.
  </phase>
